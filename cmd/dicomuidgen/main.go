...
const (
	defaultInputPath  = "internal/standard/uids.tsv"
	defaultOutputPath = "dictionary/uid/std_gen.go"
)
...
func parseArgs(args []string, stderr io.Writer) (options, error) {
	...
	fs := flag.NewFlagSet("dicomuidgen", flag.ContinueOnError)
	fs.SetOutput(stderr)
	fs.StringVar(&opts.input, "input", defaultInputPath, "path to the versioned UID TSV source file")
	fs.StringVar(&opts.output, "output", defaultOutputPath, "path to the generated Go output file")
	fs.Usage = func() {
		_, _ = fmt.Fprintf(stderr, "Usage: %s [flags]\n\nGenerate dictionary/uid from a versioned UID TSV source file.\n\nFlags:\n", fs.Name())
		fs.PrintDefaults()
	}
