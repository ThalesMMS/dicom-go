...
func TestParseTimeErr(t *testing.T) {
	badValues := []struct {
		...
		{
			Name:     "MissingHours_FractionalOnly",
			BadValue: ".123456",
		},
		{
			Name:     "Totally Wrong",
...
func TestTimeFormattingUsesSingleAllocation(t *testing.T) {
	timeValue := time.Date(1, 1, 1, 1, 2, 3, 456789000, time.UTC)
	cases := []struct {
		name string
		time dcmtime.Time
		dcm  string
		...
		{
			name: "hours",
			time: dcmtime.Time{Time: timeValue, Precision: dcmtime.PrecisionHours},
			dcm:  "01",
			text: "01",
		},
		{
			name: "seconds",
			time: dcmtime.Time{Time: timeValue, Precision: dcmtime.PrecisionSeconds},
			dcm:  "010203",
			text: "01:02:03",
		},
		{
			name: "fractional",
			time: dcmtime.Time{Time: timeValue, Precision: dcmtime.PrecisionFull},
			dcm:  "010203.456789",
	...
	for _, tc := range cases {
		t.Run(tc.name+"/DCM", func(t *testing.T) {
			assertFormatAllocs(t, 1, tc.time.DCM, tc.dcm)
		})
		t.Run(tc.name+"/String", func(t *testing.T) {
			assertFormatAllocs(t, 1, tc.time.String, tc.text)
		})
	}
}
...
func BenchmarkTimeFormatting(b *testing.B) {
	...
	cases := []struct {
		name string
		time dcmtime.Time
		fn   func(dcmtime.Time) string
	}{
		{"DCM/hours", dcmtime.Time{Time: timeValue, Precision: dcmtime.PrecisionHours}, dcmtime.Time.DCM},
		{"DCM/seconds", dcmtime.Time{Time: timeValue, Precision: dcmtime.PrecisionSeconds}, dcmtime.Time.DCM},
		{"DCM/fractional", dcmtime.Time{Time: timeValue, Precision: dcmtime.PrecisionFull}, dcmtime.Time.DCM},
		{"String/seconds", dcmtime.Time{Time: timeValue, Precision: dcmtime.PrecisionSeconds}, dcmtime.Time.String},
		{"String/fractional", dcmtime.Time{Time: timeValue, Precision: dcmtime.PrecisionFull}, dcmtime.Time.String},
	}
