...
func buildVRSequence(volumeGeneration, firstViewGeneration uint64) ([]StateStep, error) {
	...
	for index, position := range cameras {
		...
		material := dicomrender.BasicClinicalVRLightingMaterial()
		if index >= 2 {
			material = dicomrender.GlossyVascularVRLightingMaterial()
		}
		state := dicomrender.ViewState{
			ContractVersion:        dicomrender.ViewStateContractVersionV4,
			StructSize:             dicomrender.ViewStateHeaderSizeV4,
			...
			VR: dicomrender.VRViewState{
				...
				OpacityUnitDistanceMM: dicomrender.DefaultVROpacityUnitDistanceMM,
				...
				BackgroundRGBA:        [4]float64{0, 0, 0, 1},
